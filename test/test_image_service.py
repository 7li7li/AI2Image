...
class ImageServiceTests(unittest.TestCase):
    ...
    def test_list_images_rewrites_local_record_url_to_current_base_url(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            images_dir = Path(tmp_dir) / "images"
            image_path = images_dir / "2026" / "05" / "25" / "sample.png"
            image_path.parent.mkdir(parents=True)
            ...
            fake_storage = SimpleNamespace(
                load_image_records=lambda: [
                    {
                        "id": "image-1",
                        "url": "https://old.example/images/2026/05/25/sample.png",
                        "created_at": "2026-05-25 22:49:30",
                    }
                ]
            )
            ...
            with mock.patch.object(image_service, "config", fake_config):
                result = image_service.list_images("https://site.example")

        self.assertEqual(result["items"][0]["url"], "https://site.example/images/2026/05/25/sample.png")
